...
THUMBNAIL_MAX_SIZE = 800  # 缩略图最大边�?
# ============ 图片加载排队机制 ============
# 每个用户的最大并发下载数
MAX_CONCURRENT_DOWNLOADS_PER_USER = 2
# 排队超时时间（秒�?QUEUE_TIMEOUT = 60
# 每个用户的当前下载计�?user_active_downloads: Dict[str, int] = {}
# 用户队列�?queue_lock = asyncio.Lock()
...
# 初始�?- 支持直接运行和模块运�?SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    ...
    while scheduler_running:
        try:
            ...
            if current_time == target_time and last_backup_date != today_str:
                ...
                set_last_backup_date(today_str)  # 持久化到数据�?
                log_message(f"自动备份已执行，下次�?{target_time} 执行")
        
        except Exception as e:
            log_message(f"调度器异�? {str(e)}")
...
async def get_review_image(
    ...
    if not image:
        return JSONResponse(content={"message": "暂无待审核图�?, "image": None})
...
async def submit_image_review(
    ...
    if status not in ['pass', 'fail', 'skip']:
        raise HTTPException(status_code=400, detail="无效的审核状�?)
...
async def download_image(image_id: int, user_id: str = None):
    ...
    if user_id:
        ...
        try:
            ...
            conn.close()
            if not image:
                raise HTTPException(status_code=404, detail="图片不存�?)
...
def _compress_image(img, max_size=500*1024, initial_quality=85, quality_step=10,
    ...
    Args:
        ...
        initial_quality: 初始压缩质量，默�?5
        quality_step: 质量递减步长，默�?0
...
async def get_thumbnail(image_id: int):
    ...
    try:
        ...
        # 记录缩略图处理失败的异常，避免静默失�?
        logger.exception("缩略图生成失�?%s, 返回原图", original_path)
...
async def admin_create_role(
    ...
    if avatar:
        ...
        if len(avatar_content) > MAX_FILE_SIZE:
            raise HTTPException(status_code=400, detail="文件过大，最�?MB")
...
async def admin_update_role(
    ...
    if refresh_images and refresh_images.lower() == 'true':
        ...
        if not refresh_success:
            log_message(f"修改角色 {role_id} 时刷新图片失�?)
    ...
    # 返回包括刷新状态的结果
    if not refresh_success:
        return {"success": True, "refresh_success": False, "error": "角色信息已更新，但刷新图片失败，可能是路径无�?}
    return {"success": True}
...
async def admin_clear_user_reviews(user_id: str, x_admin_password: str = Header(None)):
    ...
    log_message(f"清除用户 {user_id} 的所有审核记�?)
    return {"success": True}
...
async def admin_export_disputed(x_admin_password: str = Header(None)):
    ...
    if not disputed_images:
        return JSONResponse(content={"message": "暂无可导出图�?, "count": 0})
    ...
    try:
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            log_message(f"争议图片导出完成，共 {total_count} 张图�?)
...
async def admin_update_auto_backup_enabled(enabled: str = Form(...), x_admin_password: str = Header(None)):
    ...
    log_message(f"更新自动备份启用状�? {enabled}")
    return {"success": True}
